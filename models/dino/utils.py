...
def gen_sineembed_for_width(pos_tensor):
    # n_query, bs, _ = pos_tensor.size()
    # sineembed_tensor = torch.zeros(n_query, bs, 256)
    scale = 2 * math.pi
    dim_t = torch.arange(128, dtype=torch.float32, device=pos_tensor.device)
    dim_t = 10000 ** (2 * (dim_t // 2) / 128)
    a_embed = pos_tensor[:, :, 0] * scale
    pos_a = a_embed[:, :, None] / dim_t
    pos_a = torch.stack((pos_a[:, :, 0::2].sin(), pos_a[:, :, 1::2].cos()), dim=3).flatten(2) 
    return pos_a

def gen_sineembed_for_angle(pos_tensor):
    # n_query, bs, _ = pos_tensor.size()
    # sineembed_tensor = torch.zeros(n_query, bs, 256)
    scale = 2 * math.pi
    dim_t = torch.arange(128, dtype=torch.float32, device=pos_tensor.device)
...
def gen_sineembed_for_level(pos_tensor):
    # n_query, bs, _ = pos_tensor.size()
    # sineembed_tensor = torch.zeros(n_query, bs, 256)
    scale = 2 * math.pi
    dim_t = torch.arange(128, dtype=torch.float32, device=pos_tensor.device)
