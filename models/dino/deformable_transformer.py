...
from .utils import gen_encoder_output_proposals, MLP, _get_activation_fn, gen_sineembed_for_position, gen_sineembed_for_width, gen_sineembed_for_angle, gen_sineembed_for_level
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8,
        ...
        self.enc_out_width_embed = None
        self.enc_out_angle_embed = None

        # learned angle
        self.refangle_embed = nn.Parameter(torch.zeros(bs, 100, 1))
        self.reflvl_embed = nn.Parameter(torch.full((bs, 100, 1), 0.5))
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, input_query_width=None, input_query_angle=None, input_query_level=None):
        """
        Input:
        ...
        if self.two_stage_type == 'standard':
            ...
            output_memory, output_proposals = gen_encoder_output_proposals(memory, mask_flatten, spatial_shapes,
                                                                           input_hw)
            # clone width
            new_feature = 0.5 * output_proposals[:, :, 2:3].clone()
            output_proposals = torch.cat([output_proposals, new_feature], dim=-1)

            output_memory = self.enc_output_norm(self.enc_output(output_memory))

            enc_outputs_class_unselected = self.enc_out_class_embed(output_memory)
            enc_outputs_coord_unselected = self.enc_out_bbox_embed(
                output_memory) + output_proposals[:,:,:4]  # (bs, \sum{hw}, 4) unsigmoid

            # width
            enc_outputs_width_unselected = self.enc_out_width_embed(output_memory) + output_proposals[:,:, 4:5]
            ...
            # width
            refwidth_embed_undetach = torch.gather(enc_outputs_width_unselected, 1,
                                                    topk_proposals.unsqueeze(-1).repeat(1, 1, 1))  # unsigmoid
            refwidth_embed_ = refwidth_embed_undetach.detach()

            refangle_embed_undetach = self.refangle_embed
            refangle_embed_ = refangle_embed_undetach.detach()

            reflvl_embed_undetach = self.reflvl_embed
            reflvl_embed_ = reflvl_embed_undetach.detach()
            ...
            if refpoint_embed is not None:
                ...
                refpoint_embed = torch.cat([refpoint_embed, input_query_width, input_query_angle, input_query_level], dim=-1)
                refpoint_embed_ = torch.cat([refpoint_embed_, refwidth_embed_, refangle_embed_, reflvl_embed_], dim=-1)
                refpoint_embed = torch.cat([refpoint_embed, refpoint_embed_], dim=1)
                tgt = torch.cat([tgt, tgt_], dim=1)
            else:
                refangle_embed_undetach = nn.Parameter(torch.zeros(1, 100, 1)).to(device = src.device)
                refangle_embed_ = refangle_embed_undetach.detach()
                reflvl_embed_undetach = nn.Parameter(torch.full((1, 100, 1), 0.5)).to(device = src.device)
                reflvl_embed_ = reflvl_embed_undetach.detach()
                refpoint_embed = torch.cat([refpoint_embed_, refwidth_embed_, refangle_embed_, reflvl_embed_], dim=-1)
                tgt = tgt_
        ...
        if self.two_stage_type == 'standard':
            ...
            if self.two_stage_keep_all_tokens:
                ...
                ref_enc = torch.cat([refpoint_embed_undetach, refwidth_embed_undetach, refangle_embed_undetach, reflvl_embed_undetach], dim=-1).sigmoid().unsqueeze(0)
        else:
            hs_enc = ref_enc = None
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None,
        ...
        self.ref_point_head = MLP(query_dim // 2 * d_model + 256 + 128, d_model, d_model, 2)
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.deformable_decoder:
                ...
                if reference_points.shape[-1] == 7:
                    # [N,bs,1,4]*[1,bs,4,4] -> [N,bs,4,4]
                    reference_points_input_4 = reference_points[:, :, None, :4] \
                                                    * torch.cat([valid_ratios, valid_ratios], -1)[None, :]  # nq, bs, nlevel, 4
                    reference_points_input_5 = reference_points[:, :, None, 4:5] * valid_ratios[:, :, 0:1][None, :]
                    reference_points_input_6 = reference_points[:, :, None, 5:6].expand(-1, -1, 4, -1)  # [1100, 2, 1, 1]
                    reference_points_input_7 = reference_points[:, :, None, 6:7].expand(-1, -1, 4, -1)  # [1100, 2, 1, 1]

                    reference_points_input = torch.cat([reference_points_input_4,
                                                        reference_points_input_5,
                                                        reference_points_input_6,
                                                        reference_points_input_7
                                                        ], dim=-1)
                else:
                ...
                query_sine_embed_4 = gen_sineembed_for_position(reference_points_input[:, :, 0, :4])  # nq, bs, 256*2
                query_sine_embed_5 = gen_sineembed_for_width(reference_points_input[:, :, 0, 4:5])
                query_sine_embed_6 = gen_sineembed_for_angle(reference_points_input[:, :, 0, 5:6])
                query_sine_embed_7 = gen_sineembed_for_level(reference_points_input[:, :, 0, 6:7])
                query_sine_embed = torch.cat([query_sine_embed_4, query_sine_embed_5, query_sine_embed_6, query_sine_embed_7], dim=-1)
            else:
            ...
            if self.bbox_embed is not None:
                ...
                delta_lvl_unsig = self.level_embed[layer_id](output)
                delta_width_unsig = self.width_embed[layer_id](output)
                delta_angle_unsig = self.angle_embed[layer_id](output)
                outputs_unsig = delta_unsig + reference_before_sigmoid[:, :, :4]
                outputs_width_unsig = delta_width_unsig + reference_before_sigmoid[:, :, 4:5]
                outputs_lvl_unsig = delta_lvl_unsig + reference_before_sigmoid[:, :, 6:7]

                outputs_angle_unsig = delta_angle_unsig + reference_before_sigmoid[:, :, 5:6]
                outputs_unsig = torch.cat([outputs_unsig, outputs_width_unsig, outputs_angle_unsig, outputs_lvl_unsig], dim=-1)
